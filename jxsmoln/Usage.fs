...
let usage command =
  cp "\foConvert between JSON and JXSMOLN (XML)\f0"
  cp ""
  cp "\fojxsmoln \fg-j \fcfile.json\f0 [\fg-o \fcfile.xml\f0] [\fg-mjson\f0]"
  cp "   Convert JSON to JXSMOLN"
  cp "   \fg-j \fcfile.json\f0    The input JSON file. Supports both normal *.json files as well as *.mjson and *.jsonl files."
